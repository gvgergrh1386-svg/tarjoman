"""Installed Chrome 3.7.6 smoke: upgrade data, runtime witness, themes and workshop.
...
python dev/release_smoke_376.py --extension PATH --upgrade-from PATH --output PATH
"""
import argparse
import base64
import contextlib
...
parser.add_argument('--expected-version', default='3.7.6')
...
def page(ws, ext_id, path):
    ...
    ws.call('Page.addScriptToEvaluateOnNewDocument', {'source': "Object.defineProperty(navigator,'languages',{get:()=>['fa-IR']});Object.defineProperty(navigator,'language',{get:()=>'fa-IR'});"}, session=session)
...
def runtime_witness(ws, ext_id):
    for attempt in range(50):
        ...
        if target:
            ...
            return evaluate(ws, session, "({version:chrome.runtime.getManifest().version,stableTweets:typeof translateStableTweets==='function',workshop:typeof translateWorkshop==='function',siteMutation:typeof GXTBG.setWebVideoSiteBlocked==='function',youtubeSettings:typeof GXTBG.youtubeSettings==='function',promptVersion:GXTBG.subtitlePrompts?.VERSION||null})")
        time.sleep(.1)
...
try:
    ...
    with tempfile.TemporaryDirectory(prefix='gxt-376-release-smoke-') as temp_name:
        temp = Path(temp_name)
        ...
        with browser(profile, stage) as (ws, old_id):
            ...
            seeded = evaluate(ws, session, """(async()=>{
              ...
              return chrome.runtime.getManifest().version===EXPECTED_BASELINE && (await GXT.getSettings()).pageBilingual;
            })()""".replace('EXPECTED_BASELINE', json.dumps(json.loads((args.upgrade_from/'manifest.json').read_text('utf8'))['version'])))
            check('baseline runtime seeds synthetic settings and memory', seeded)
        copy_runtime(args.extension, stage)
        with browser(profile, stage) as (ws, new_id):
            native_reload(ws,new_id)
            time.sleep(.5)
            ...
            activated = witness.get('version') == expected and all(witness.get(k) for k in ['stableTweets','workshop','siteMutation','youtubeSettings','promptVersion'])
            ...
            if not activated:
                try:
                    ...
                    activated = reloaded_id == new_id and witness.get('version') == expected and all(witness.get(k) for k in ['stableTweets','workshop','siteMutation','youtubeSettings','promptVersion'])
        ...
        with browser(clean_profile, stage) as (ws, ext_id):
            ...
            check('fresh worker executes new 3.7.6 functions, beyond manifest metadata', witness.get('version') == expected and all(witness.get(k) for k in ['stableTweets','workshop','siteMutation','youtubeSettings','promptVersion']))
...
print(f'RELEASE-SMOKE-376 SUMMARY {passed}/{len(checks)}', flush=True)
